...
class ProductsListSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, obj):
        serialized_data = super(ProductsListSerializer,
                                self).to_representation(obj)
        product_id = serialized_data.get('id')
        if self.context.get("userid"):
            try:
                customercart = CustomerCart.objects.get(
                    product_id=product_id, customer_id=self.context.get("userid"))
                serialized_data['incart'] = 1
            except:
                serialized_data['incart'] = 0
        else:
            serialized_data['incart'] = 0
            print("aaaa", serialized_data)
